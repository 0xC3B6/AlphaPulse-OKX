...
describe("App", () => {
  ...
  it("shows Review performance and trade records without Monitor filters", async () => {
    ...
    const strategyCurve = screen.getByTestId("paper-strategy-curve");
    expect(within(strategyCurve).getByRole("button", { name: "1D" })).toBeInTheDocument();
    expect(within(strategyCurve).getByRole("button", { name: "7D" })).toBeInTheDocument();
    expect(within(strategyCurve).getByRole("button", { name: "ALL" })).toBeInTheDocument();
    const strategyCurveChart = screen.getByTestId("paper-strategy-recharts");
    expect(strategyCurveChart).toBeInTheDocument();
    expect(screen.getByTestId("paper-strategy-axis-summary")).toHaveTextContent("时间轴");
    fireEvent.click(within(strategyCurve).getByRole("button", { name: "1D" }));
    expect(screen.getByTestId("paper-strategy-axis-summary")).toHaveTextContent("时间轴 · 10分钟");
    expect(strategyCurveChart).toHaveAttribute("data-axis-step-ms", String(10 * 60 * 1_000));
    fireEvent.click(within(strategyCurve).getByRole("button", { name: "7D" }));
    expect(screen.getByTestId("paper-strategy-axis-summary")).toHaveTextContent("时间轴 · 1小时");
    expect(strategyCurveChart).toHaveAttribute("data-axis-step-ms", String(60 * 60 * 1_000));
    fireEvent.click(within(strategyCurve).getByRole("button", { name: "30D" }));
    expect(screen.getByTestId("paper-strategy-axis-summary")).toHaveTextContent("时间轴 · 4小时");
    expect(strategyCurveChart).toHaveAttribute("data-axis-step-ms", String(4 * 60 * 60 * 1_000));
    fireEvent.click(within(strategyCurve).getByRole("button", { name: "90D" }));
    expect(screen.getByTestId("paper-strategy-axis-summary")).toHaveTextContent("时间轴 · 12小时");
    expect(strategyCurveChart).toHaveAttribute("data-axis-step-ms", String(12 * 60 * 60 * 1_000));
